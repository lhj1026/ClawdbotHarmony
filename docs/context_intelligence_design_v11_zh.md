...
> 变更: v11 移除了旧版规则引擎内的 Stream MLP、LinUCB (14维)、MAB 及 LocationDiscovery 模块（已在 v2.66.1 中移除，由 ActionRecommender 替代）。
...
> **注**: 旧版规则引擎内的在线学习模块（Stream MLP 34维、LinUCB 14维、MAB epsilon-greedy、LocationDiscovery GPS 聚类）已在 v2.66.1 中移除。动作推荐学习功能现由 **ActionRecommender** (`action_recommender.h`) 统一承载，采用 MLP + LinUCB 185 维特征向量，详见英文版相关章节。
...
这些 `ps_*` 键对 C++ 规则引擎的规则匹配可用，同时也作为 **ActionRecommender** 185 维特征向量的输入来源。

> **注**: 旧版规则引擎内嵌的 Stream MLP `buildFeatures()` (34维) 和 LinUCB `buildFeatureVec()` (14维) 已在 v2.66.1 中移除。在线学习与动作推荐功能现统一由 ActionRecommender (`action_recommender.h`) 承载，采用 MLP + LinUCB 185 维特征向量。
...
| ~~`stream_mlp.h`~~ | ~~已移除~~ | ~~旧版 Stream MLP 34维在线学习~~ — 已在 v2.66.1 中移除，由 ActionRecommender 替代 |
| ~~`stream_mlp.cpp`~~ | ~~已移除~~ | ~~旧版 `buildFeatures()` 34维特征构建~~ — 已在 v2.66.1 中移除，由 ActionRecommender 替代 |
| ~~`context_engine.h`~~ | ~~已移除~~ | ~~旧版 `LINUCB_DIM` 8→14~~ — 已在 v2.66.1 中移除，由 ActionRecommender 替代 |
| ~~`linucb.cpp`~~ | ~~已移除~~ | ~~旧版 `buildFeatureVec()` 14维~~ — 已在 v2.66.1 中移除，由 ActionRecommender 替代 |
